...
export default function WeeklyExpenses() {
    ...
    useEffect(() => {
        async function fetchWeeklyExpenses() {
            try {
                const response = await fetch("/api/expenses/get-weekly-expenses", { cache: 'no-store'});
    ...
    const filteredExpenses = weeklyExpenses.filter(expense => 
        expense.expense_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        `#w-exp-${expense.expense_item_id}`.includes(searchQuery.toLowerCase())
    );
            ...
            <div className="max-w-7xl mx-auto space-y-8 py-10 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 dark:border-slate-800 pb-6 gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Review operational expenditure metrics for the current week cycles.</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/60 rounded-2xl p-5 flex items-center gap-4 shadow-sm min-w-[280px]">
                        ...
                        <div>
                            <p className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Weekly Total</p>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-2xl shadow-xl shadow-slate-200/20 dark:shadow-none overflow-hidden">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 space-y-4">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Aggregating weekly expenditure statements...</p>
                        </div>
                    ) : filteredExpenses.length === 0 ? (
                        <div className="text-center py-20 text-slate-400 dark:text-slate-500">
                            <span className="text-5xl block mb-4">📭</span>
                            <p className="text-base font-semibold">No expenses documented for this weekly bracket.</p>
                                ...
                                <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                                    {filteredExpenses.map((expense) => (
                                        <tr key={expense.expense_item_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/40 transition-colors">
                                            <td className="px-6 py-4 font-mono text-xs text-slate-400 font-semibold">#W-EXP-{expense.expense_item_id}</td>
