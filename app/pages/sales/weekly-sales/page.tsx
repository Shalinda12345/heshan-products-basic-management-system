...
export default function WeeklySales() {
  ...
  const totalRevenue = weeklySales.reduce((sum, s) => sum + Number(s.grand_total), 0);
    ...
    <main className="min-h-screen bg-slate-50/50 dark:bg-slate-900 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 dark:border-slate-800 pb-5 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Weekly Statements</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review operational performance metrics for the current week cycles.</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-center gap-4 shadow-sm min-w-[240px]">
            <div className="p-3 bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 rounded-lg text-xl">📈</div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">This Week's Revenue</p>
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/60 rounded-xl shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-3">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Aggregating weekly arrays...</p>
            </div>
          ) : weeklySales.length === 0 ? (
            <div className="text-center py-16 text-slate-500 dark:text-slate-400">
              <p className="text-lg font-medium">No sales operations logged for this weekly bracket.</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                  {weeklySales.map((sale) => (
                    <tr key={sale.sale_id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/40 transition-colors">
                      <td className="px-6 py-4 font-mono text-xs text-slate-400">#W-SAL-{sale.sale_id}</td>
